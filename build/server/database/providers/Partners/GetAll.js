...
const getAll = (page_1, limit_1, filter_1, IdUser_1, ...args_1) => __awaiter(void 0, [page_1, limit_1, filter_1, IdUser_1, ...args_1], void 0, function* (page, limit, filter, IdUser, id = 0) {
    try {
        const result = yield (0, knex_1.Knex)(ETablesNames_1.EnumTableNames.partners)
            .select('partners.*', 'address.id as address_id', // Address
        'address.street', 'address.number', 'address.complement', 'address.city', 'address.state', 'address.zip_code', 'address.created_at as address_created_at', 'address.updated_at as address_updated_at')
            .leftJoin(ETablesNames_1.EnumTableNames.address, 'partners.id', 'address.id_partner')
        ...
        if (id > 0 && result.every((item) => item.id !== id)) {
            const resultById = yield (0, knex_1.Knex)(ETablesNames_1.EnumTableNames.partners)
                .select('*')
                .where('id', '=', id)
                .first();
            if (resultById)
