...
const getAll = (page_1, limit_1, filter_1, IdUser_1, ...args_1) => __awaiter(void 0, [page_1, limit_1, filter_1, IdUser_1, ...args_1], void 0, function* (page, limit, filter, IdUser, id = 0) {
    try {
        const result = yield (0, knex_1.Knex)(ETablesNames_1.EnumTableNames.registrations)
            .select('*')
            .where('id', id)
            .orWhere('number_record', 'like', `%${filter}%`)
            .andWhere('id_user', IdUser)
            .offset((page - 1) * limit)
            .limit(limit);
        if (id > 0 && result.every((item) => item.id !== id)) {
            const resultById = yield (0, knex_1.Knex)(ETablesNames_1.EnumTableNames.registrations)
                .select('*')
                .where('id', '=', id)
                .first();
            if (resultById)
